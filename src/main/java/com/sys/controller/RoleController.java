...
public class RoleController {
	...
	public PageResult list(PageTableRequest request) {
		...
		request.countOffset();
		return roleService.getAllRolesByPage(request.getPage()-1, request.getRows());
	}
    ...
    @ApiOperation(value = "批量删除角色信息", notes = "批量删除角色信息")//描述
    public Results<SysRole> deleteRole(@RequestBody RoleDto roleDto) {
        return roleService.deleteByRoleIds(roleDto.getRoleIds());
    }
    ...
    public Results findRoleByFuzzyRoleName(PageTableRequest requests, String roleName) {
        ...
        return roleService.getRoleByFuzzyRoleNamePage(roleName,requests.getOffset(),requests.getRows());
    }
